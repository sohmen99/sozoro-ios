...
**ウェブ版の `refreshHeat` をそのまま移してあります。**
...
```
 3時  最濃 秋葉原 37   半径565m 濃さ0.23 緑     緑21 / 琥珀0 / 朱0
14時  最濃 浅草寺 100  半径928m 濃さ0.44 朱     緑4 / 琥珀10 / 朱7
20時  最濃 アメ横 95   半径899m 濃さ0.42 朱     緑15 / 琥珀3 / 朱3
```

**時間帯の変化は実測ではありません。**国交省の人流メッシュが持っているのは
休日昼・平日昼・全日夜の3値で、いま焼き込んでいるのは昼の2値だけです。
24時間のかたち（層3）と施設ごとの集客力（層2）は推定です。実測が効いているのは
「その一帯にどれくらい人がいるか」という空間の分だけ。ウェブ版と同じ扱いです。
...
**色は熱の階調で持ちます。**地点の印に使う三段（空き=緑・ふつう=琥珀・混雑=朱）を
にじみにも流用していたせいで、空いている時間帯は地図が緑に沈んでいました。
ヒートは別の色として持ちます。

```
比 1.00  赤紫 #8E1E5E   ← いちばん混んでいるところ
   0.80  朱   #C63A2C
   0.60  橙   #DD6B2B
   0.35  山吹 #E8A93C
   0.00  黄   #F0DC72   ← 薄く消えていく
```

濃さは二段階で決めます。**その時刻の最大との比**で個々の濃さを出し、
さらに**その時刻がそもそも混んでいるか**で層全体を抑える。

絶対値だけで切ると深夜に層ごと消えます（0〜6時と23時は全21地点が45を割る）。
比だけで描くと、逆に3時の秋葉原（絶対値37＝空いている）が全力の赤紫になって嘘になる。
両方かけると、昼は濃さ0.50、深夜は0.26で「薄いけれどどこが相対的に濃いかは分かる」
状態に落ち着きます。

デモで時刻を動かすと、にじみも基準地点の点も入れ替わります。
